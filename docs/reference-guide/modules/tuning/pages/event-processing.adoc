...
Event processing in Axon Framework 5 is centered around `PooledStreamingEventProcessor`.
...
If you prefer Spring Boot auto-configuration, expose the same processor settings as beans or let the framework bind its defaults and only override the values you need.
...
* size the database so token claims and event handling do not contend for connections
...
In an aggregate-based store, the sequencing and token management story is still centered on aggregate-oriented ordering.
...
* Enable coordinator claim extension when workers process large batches and token claims need to be refreshed more aggressively.
