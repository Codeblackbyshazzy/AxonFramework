...
There are three approaches to registering interceptors with your event handling processes, each with a different scope.
...
Secondly, interceptors can be registered for all event handling components within a specific processor's configuration, using `intercepted()` on the `EventHandlingComponentsConfigurer`.
...
Thirdly, interceptors can be registered for a specific Event Processor instance by name.
For this, use the `withInterceptor(MessageHandlerInterceptor<? super EventMessage>)` operation as part of customizing a processor:
