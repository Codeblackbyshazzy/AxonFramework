...
[NOTE]
====
In most scenarios, you do not need to invoke `start()` or `shutdown()` manually.
Axon Framework manages the processor lifecycle automatically as part of application startup and shutdown.
Only call these methods directly when you need explicit control, for example when triggering a programmatic replay or conditionally pausing processing.
====
