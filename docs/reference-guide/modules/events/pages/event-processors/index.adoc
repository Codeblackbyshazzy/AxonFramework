...
Every `EventProcessor` exposes a common set of lifecycle methods to start, stop, and inspect state.
These methods are available on all processor implementations, including the xref:event-processors/subscribing.adoc[Subscribing] and xref:event-processors/streaming.adoc[Streaming] Event Processor.

=== Starting and stopping

`start()` and `shutdown()` both return a `CompletableFuture<Void>`.
The future completes when the operation has fully finished, allowing callers to chain further work or simply await it:
...
public class ProcessorLifecycleService {
    ...
    public ProcessorLifecycleService(AxonConfiguration configuration) {
        this.configuration = configuration;
    }

    public void startAndAwait(String processorName) {
        EventProcessor processor = configuration.getComponent(EventProcessor.class, processorName);
        // Blocks until the processor is fully started
        processor.start().join();
    }

    public void stopAndAwait(String processorName) {
        EventProcessor processor = configuration.getComponent(EventProcessor.class, processorName);
        // Blocks until the processor is fully stopped
...
[NOTE]
====
What "fully started" means depends on the processor implementation.
For the xref:event-processors/streaming.adoc[Pooled Streaming Event Processor], `start()` completes only after the token store identifier has been resolved and the coordinator has started.
Do not call processor-specific methods that require an active processor (such as `getTokenStoreIdentifier()`) before awaiting `start()`.
====
...
* `isRunning()`: returns `true` when the processor is active and processing events.
...
include::dead-letter-queue:partial$event-processors-dlq.adoc[]
endif::[]
...
Default handling interceptors can be attached to all Event Processors in your application through the `MessagingConfigurer`.
This is explained in more detail xref:messaging-concepts:message-intercepting.adoc#event-handler-interceptors[here].

==== Interceptors for a specific processor instance

Interceptors can be registered for a specific Event Processor instance by name.
Use the `withInterceptor(MessageHandlerInterceptor<? super EventMessage>)` operation as part of customizing a processor:
...
Interceptors can be registered as part of the event handling component configuration itself, using `intercepted()` on the `EventHandlingComponentsConfigurer`.
Calling `intercepted()` closes the component registration phase, so all components must be registered before calling it.
Multiple `intercepted()` calls accumulate interceptors in registration order:
...
Component registration ends when the first `.intercepted(...)` is called.
Additional interceptors can still be added with further `.intercepted(...)` calls and are applied in registration order.

==== Annotated interceptors on the handler class

Methods on the handler class itself can be annotated with `@EventHandlerInterceptor`.
These methods are automatically discovered and run before every `@EventHandler` method on the same component instance, making them ideal for cross-cutting concerns that share the component's state (such as tenant filtering or component-level auditing).

Two styles are supported:

* A *before-interceptor* is a `void` method with no `MessageHandlerInterceptorChain` parameter.
  It runs before the handler, and the chain automatically proceeds when the method returns normally.
  If the method throws, the handler is not invoked.
* A *surround-interceptor* declares a `MessageHandlerInterceptorChain` parameter and returns a `MessageStream`.
  It controls whether the handler is invoked by calling `chain.proceed(event, ctx)`.
  Returning without calling `proceed` short-circuits the chain.
