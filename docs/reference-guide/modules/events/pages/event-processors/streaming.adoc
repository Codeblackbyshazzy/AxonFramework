...
The `PooledStreamingEventProcessor` follows the common xref:event-processors/index.adoc#lifecycle[Event Processor lifecycle] contract, with one important addition: `start()` performs initialization work before event processing can begin.
...
public class ProcessorService {
    ...
    public void startAndQueryIdentifier(StreamingEventProcessor processor) {
        processor.start()
                 .thenRun(() -> {
                     // Safe: start() has completed, identifier is resolved
...
[IMPORTANT]
====
`getTokenStoreIdentifier()` throws an `IllegalStateException` if called before `start()` has completed.
Always await the `start()` future before calling it.
====
...
3. <<event_processor_definition_based_config,`EventProcessorDefinition` configuration>>, like Spring
...
[#event_processor_definition_based_config]
...
3. Handlers not matched by any `EventProcessorDefinition` are assigned to a processor based on their package name (default behavior)
...
This identifier is resolved during `start()` as part of the processor's startup sequence and can be queried with `getTokenStoreIdentifier()` once the processor has started.

[IMPORTANT]
====
`getTokenStoreIdentifier()` requires the processor to have been started.
Calling it before `start()` has completed throws an `IllegalStateException`.
====
...
public class AxonConfig {
    ...
    public String tokenStoreFor(StreamingEventProcessor eventProcessor) {
        // The processor must be started before calling getTokenStoreIdentifier()
...
public interface SequencingPolicy {
    ...
    Optional<Object> sequenceIdentifierFor(EventMessage event, ProcessingContext context);
}
----
