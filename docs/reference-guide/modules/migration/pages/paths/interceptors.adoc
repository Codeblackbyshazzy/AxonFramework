...
The `payloadType` attribute narrows the interceptor to a specific payload type:
...
The main change is that exception handler methods no longer receive `CommandMessage` as a parameter to identify the command context; instead, use the `payloadType` attribute.
...
public class OrderCommandHandler {
    ...
    // Narrow to a specific command type using payloadType
    @ExceptionHandler(
        resultType = IllegalStateException.class,
        payloadType = PlaceOrderCommand.class
    )
    public void handlePlaceOrderIllegalState(IllegalStateException exception) {
        // Only handles exceptions from PlaceOrderCommand handlers
    }
}
----
====
