...
[TIP]
.Reactive command dispatching
====
If your application uses Project Reactor (for example, Spring WebFlux), the xref:ROOT:modules.adoc#axon-reactor[Reactor extension] provides a `ReactorCommandGateway` that returns `Mono<R>` instead of `CompletableFuture`.
Reactive dispatch interceptors run inside the Reactor subscription, enabling access to Reactor context such as `ReactiveSecurityContextHolder`.
====
...
When using the property name for a method, the framework resolves it by looking for methods starting with `get`, `is`, or matching the name exactly.
